...
48_refresh_gold.py
-------------------------------------------------------------------
The frequent, lightweight refresh job for Gold (asset_type='gold'),
separate from 05_daily_price_refresh.py (which only loops over
asset_type='equity') so this can run on its own schedule without
touching the already-working equity refresh job. Same idea as
46_refresh_benchmark_indices.py, just for the Gold row 47_add_gold_
asset.py created.

Same "skip outside market hours unless --force" protection, and same
"a failure never wipes anything" behavior, as every other refresh
script in this project.

Run manually:
    python 48_refresh_gold.py
    python 48_refresh_gold.py --force
-------------------------------------------------------------------
"""
import sys
