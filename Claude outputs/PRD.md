...
- **Status: BUILT (Session 12).** `/ipos` (list) and `/ipos/[ipoId]` (detail) live, backed by real NSE data (`34_refresh_ipos.py`) — SME-platform IPOs (NSE Emerge) are explicitly excluded, mainboard only. DRHP assessments are populated by a real, working PDF-parsing pipeline (`37_parse_drhp.py`) rather than by hand: it reads each DRHP's own Table of Contents and fuzzy-matches section titles (tolerant of each merchant banker's own wording/formatting) to extract the Offer Document Summary's business/industry text, objects of the offer, top risk factors, and a financial summary table — proven against one real DRHP (Pranav Constructions) before being run across the rest. GMP is deliberately left null (no reliable official source — see script docstring for why this isn't guessed at). Visual/UI polish for these two pages (logos, richer card layout — flagged by Avdhoot as "extremely boring") is still outstanding.
