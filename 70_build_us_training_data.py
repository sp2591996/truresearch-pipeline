"""
70_build_us_training_data.py
-------------------------------------------------------------------
Phase 2 (US ML model), Step 1: build a US training dataset from our
Supabase database, following the exact same methodology already
proven out for India in 09_build_training_data.py -- just scoped to
`market = "usa"` and benchmarked against the S&P 500 instead of the
Nifty 100.
  ...
  - Fundamentals looked up "point-in-time correct" -- for any historical
    ...
    + 120 days has ALREADY PASSED as of that date, so the model never
    accidentally "sees the future" during training.
  - Label = "did this stock beat the S&P 500 benchmark over the NEXT
  ...
  - A snapshot row is KEPT even when fundamentals aren't usable yet for
    that stock at that point in time (technical-only row, fundamentals
    columns left blank) -- same fix applied to the India version, so
    early-history rows aren't thrown away for no reason.

Output: writes "us_training_dataset.csv" in this same folder (a
DIFFERENT file from India's "training_dataset.csv" -- the two markets
never share a training file). This is a one-time, throwaway
intermediate file used only to train the model in the next script.

Run with (after 68_backfill_us_price_history.py and
69_backfill_us_fundamentals.py have completed):
    python 70_build_us_training_data.py
-------------------------------------------------------------------
"""
import pandas as pd
...
MARKET = "usa"
OUTPUT_FILE = "us_training_dataset.csv"
FORWARD_WINDOW_DAYS = 63   # ~3 trading months, matches the India model
...
FISCAL_LAG_DAYS = 120      # a fiscal year's results aren't public until ~120 days after year-end -- US companies file 10-Ks similarly promptly, so this is kept the same as India
...
def main():
    ...
    print("Loading S&P 500 benchmark price history...")
    bench_asset = (
    ...
    if not bench_asset.data:
        print("ERROR: S&P 500 benchmark asset not found. Run 68_backfill_us_price_history.py first.")
        return
    ...
    print(f"Building training rows for {len(assets)} US stocks...\n")
    ...
    print(f"\nDone! Built {len(training_df)} training rows across {len(assets)} US stocks.")
